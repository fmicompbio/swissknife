...
#'
#' @description Calculate the frequencies of same strand alignment
#'     distances, for example from MNase-seq data to estimate
#'     nucleosome repeat length.
#'
#' @author Michael Stadler
#'
#' @param fname \code{character} vector with one or several bam files. If multiple
#'     files are given, distance counts from all will be summed.
#' @param regions \code{GRanges} object. Only alignments falling into these regions
#'     will be used. If \code{NULL} (the default), all alignments are used.
#' @param rmdup \code{logical(1)} indicating if duplicates should be removed. If \code{TRUE}
#'     (the default), only one of several alignments starting at the same
#'     coordinate is used.
#' @param dmax \code{numeric(1)} specifying the maximal distance
#'     between same strand alignments to count.
#'
#' @return \code{integer} vector with \code{dmax} elements, with the element at position
#'     \code{d} giving the observed number of alignment pairs at that distance.
#'
#' @references Phasograms were originally described in Valouev et al., Nature 2011
#'     (doi:10.1038/nature10002).
#'     The implementation here differs in two ways from the original algorithms:
#'     1. It does not implement removing of positions that have been seen less than
#'        \code{n} times (referred to as a \code{n}-pile subset in the paper).
#'     2. It does allow to retain only alignments that fall into selected genomic
#'        intervals (\code{regions} argument).
#'
#' @seealso \code{\link{estimateNRL}} to estimate the nucleosome repeat length from a phasogram,
#'     \code{\link{plotPhasogram}} to visualize an annotated phasogram
#'
#' @examples
...
#'
#' @description Estimate the nucleosome repeat length (NRL) from the
#'     frequencies of same-strand alignment distances (phasogram), e.g.
#'     generated by \code{\link{calcPhasogram}}. The NRL is obtained from the
#'     slope of a linear fit to the modes in the phasogram.
#'
#' @author Michael Stadler
#'
#' @param x \code{numeric} vector giving the counts of alignment distances (typically
#'     the output of \code{\link{calcPhasogram}}.
#' @param mind \code{integer(1)} specifying the minimal distance to be used for NRL estimation.
#'     The default value (140) ignores any distance too short to span at least a single
#'     nucleosome.
#' @param usePeaks \code{integer} vector selecting the modes (peaks) in the phasogram
#'     used in NRL estimation.
...
#'
#' @seealso \code{\link{calcPhasogram}} to calculate the phasogram from alignments,
#'     \code{\link{plotPhasogram}} to visualize an annotated phasogram
#'
...
#'
#' @export
estimateNRL <- function(x, mind = 140L, usePeaks=1:8, span1=100/length(x), span2=1500/length(x)) {
    stopifnot(is.numeric(x))

    if (all(x == 0)) {
        warning("NRL not estimated (phasogram contains only zeros)")
        return(list(nrl = NA, nrl.CI95 = NA, xs = NA, loessfit = NA, lmfit = NA,
                    peaks = NA, mind = mind, span1 = span1, span2 = span2, usePeaks = usePeaks))
    }
...
#' @param x \code{numeric} vector giving the counts of alignment distances (typically
#'     the output of \code{\link{calcPhasogram}}.
#' @param hide If \code{TRUE} (the default), hide phasogram counts not used in the
#'     NRL estimate (\code{mind} parameter from \code{\link{estimateNRL}}).
#' @param xlim \code{numeric(2)} with the x-axis (phase) limits in the first two plots
#'     (see Details). if \code{NULL} (the default), the full range defined by
#'     \code{x} and \code{hide} will be used.
#' @param verbosePlot If \code{TRUE}, create three plots instead of just a single plot
#'     (see Details).
#' @param ... Additional arguments passed to \code{\link{estimateNRL}} to control NRL estimation.
#'
#' @details The function will visualize an annotated phasogram. For \code{verbosePlot=FALSE}
#'     (the default), it will create a single annotated plot. For \code{verbosePlot=TRUE},
#'     it will create three plots (using \code{par(mfrow=c(1,3))}):
#'     \enumerate{
...
#'     }
#'
#' @return The return value from the call to \code{\link{estimateNRL}} (invisibly).
#'
#' @seealso \code{\link{calcPhasogram}} to calculate the phasogram from alignments,
#'     \code{\link{estimateNRL}} to estimate nucleosome repeat length
#'
