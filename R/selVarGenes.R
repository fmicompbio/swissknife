...
#' @author Dania Machlab 
#' 
#' @param sce \code{Single Cell Experiment} object containing the raw counts and the scaling factors.
...
#' @export
#' 
selVarGenes <- function(sce, Nmads = 3, minCells = 5, minExpr = 1, topExprPerc = 0.01, span = 0.2, 
                        family = "symmetric", control=loess.control(surface = "direct"), binBreaks=100, 
  ...
  lo <- stats::loess(logCV ~ logMean, span = span, family = family, control = control, subset = points_for_loess, ...)
