test_that("exampleData() works properly", {

    # arguments
    expect_error(exampleData(FALSE), "name")
    expect_error(exampleData(c("mycars", "mycars_summary")), "name")
    expect_error(exampleData("error"), "must be one of")
    expect_error(exampleData("mycars", "error"), "envir")
    expect_error(exampleData("mycars", globalenv(), "error"), "verbose")

    # listing
    expect_output(df0 <- exampleData("list"))
    expect_is(df0, "data.frame")
    expect_identical(colnames(df0),
                     c("name", "created", "description", "is_collection"))

    # return value
    expect_message(df1 <- exampleData("mycars", verbose = TRUE))
    ...
    expect_message(df2 <- exampleData("mycars_summary", verbose = TRUE))
    ...
    df3 <- exampleData("2023-05", verbose = FALSE)
    ...
    expect_message(df4 <- exampleData("new", verbose = TRUE))
    ...
    # environment
    myenv <- new.env()
    df1 <- exampleData("mycars", envir = myenv, verbose = FALSE)
    expect_true(!exists("mycars", envir = globalenv()))
